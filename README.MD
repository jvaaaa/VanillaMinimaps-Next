...
> **Fork notice:** This repository is a fork of [JNNGL/VanillaMinimaps](https://github.com/JNNGL/VanillaMinimaps),
> maintained by [@jvaaaa](https://github.com/jvaaaa). Modifications since 2026 include updating
> compatibility to Minecraft 1.21.8 / 1.21.10 / 1.21.11, fullscreen bug fixes, and permission API
> updates. All modifications are licensed under the original GPL v3 (see `LICENSE`).
...
**Supported server version: 1.21.11+**
...
The resource pack source is in `resourcepack/`.
Minecraft targeting is configured in `gradle.properties` via `minecraftVersion` and `resourcepackPackFormat`.
Build a zip with:

```bash
./gradlew resourcepackZip
```

The output is written to `build/resourcepack/`.

Shader sources are mirrored in `src/main/resources/shaders` for reference.
To sync them from the resource pack and verify they match:
